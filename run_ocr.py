...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("오류: 이미지 폴더 경로가 전달되지 않았습니다.")
        sys.exit()
    ...
    print(f"'{target_directory}' 및 모든 하위 폴더에서 이미지 파일을 검색합니다...")
    image_extensions = ['*.jpg', '*.jpeg', '*.png', '*.bmp']
    image_files = []
    for ext in image_extensions:
        # 이 부분이 모든 하위 폴더를 검색하여 빈 폴더는 알아서 건너뛰게 해줍니다.
        search_path = os.path.join(target_directory, '**', ext)
        image_files.extend(glob.glob(search_path, recursive=True))
    
    image_files = sorted(image_files)

    # 이 부분이 검색 결과 이미지 파일이 하나도 없을 경우를 처리해줍니다.
    if not image_files:
        print(f"오류: '{target_directory}' 폴더 및 하위 폴더에서 이미지 파일을 찾을 수 없습니다.")
    else:
        print(f"총 {len(image_files)}개의 이미지 파일을 찾았습니다. 순차 처리를 시작합니다.")
        print("-" * 40)
